...
function Harness() {
  ...
  const [previewTypography, setPreviewTypography] = useState<PreviewTypography>(() =>
    parseStoredTypography(localStorage.getItem(PREVIEW_TYPE_KEY)),
  );
  ...
  return (
    <>
      ...
      {/* 真实 CodeMirror 编辑器：用来核对光标颜色、选区字体与字体下拉。
          ...
          下拉位置由 portal 计算，必须看浏览器里的真实计算样式与坐标。 */}
      ...
      <SettingsPage
        ...
        onChangePreviewElement={(element, next) =>
          setPreviewTypography((prev) => ({ ...prev, [element]: next }))
        }
        onResetPreviewTypography={() => setPreviewTypography(defaultPreviewTypography())}
      />
    </>
  );
}
